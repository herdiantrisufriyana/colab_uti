...
if(all(c(2,3)%in%comp_req)){ # 10s
  impute_model_d3=
    ...
    .[partition_d3$training,c('outcome',cand_predictors_d3)] %>%
    outliers_to_missing() %>%
  ...
  imputed_d3=
    ...
    lapply(\(x)
      whole_data_d3$prediction %>%
        ...
        .[x,c('outcome',cand_predictors_d3)]
    ) %>%
...
if(all(c(2,3)%in%comp_req)){ # 20m 13s
  impute_model_d=
    ...
    .[partition_d$training,c('outcome',cand_predictors_d)] %>%
    outliers_to_missing() %>%
    mice(m=10,method='pmm',seed=seed)
...
```{r Plot selected range-span, echo=FALSE, fig.height=6, fig.width=12}
ggarrange(
  null_surv_d3
  ,null_surv_d
  ,ncol=2
...
identify_adj_vars=
  function(data,inter_predictor_assoc){
    data %>%
      colnames(.) %>%
      .[!.%in%c('km_outcome','outcome','outcome_onset')] %>%
      `names<-`(
...
adjustment_variable_d3=
  ...
  numerize_scale(range_span_d3,cand_predictors2_d3,scaler_d3) %>%
  mutate(km_outcome=Surv(outcome_onset,outcome)) %>%
  identify_adj_vars(inter_predictor_assoc_d3) %>%
  lapply(\(x)x[-1]) %>%
...
adjustment_variable_d=
  ...
  numerize_scale(range_span_d,cand_predictors2_d,scaler_d) %>%
  mutate(km_outcome=Surv(outcome_onset,outcome)) %>%
  identify_adj_vars(inter_predictor_assoc_d) %>%
  lapply(\(x)x[-1]) %>%
  sapply(paste0,collapse='+') %>%
  data.frame(adjustment_variables=.) %>%
  rownames_to_column(var='predictor')
...
```{r}
imputed_d3 %>%
  lapply(select_at,c('outcome','outcome_onset',cand_predictors3_d3)) %>%
  lapply(numerize_scale,range_span_d3,cand_predictors3_d3,scaler_d3)
...
univar_surv_d$BMI_GT30=
  imputed_d$training %>%
  filter(outcome_onset<=range_span_d$r) %>%
  mutate(outcome_onset=floor(outcome_onset/range_span_d$s)) %>%
  numerize_scale(range_span_d,cand_predictors3_d,scaler_d) %>%
  ggsurvplot(
    fit=survfit(Surv(outcome_onset,outcome)~BMI_GT30,data=.)
    ,data=.
    ,risk.table=T
    ,pval=F
    ,conf.int=T
...
```{r Plot KM sel. cand. predictors, echo=FALSE, fig.height=36, fig.width=18}
ggarrange(
  null_surv_d3
  ,univar_surv_d3$prev_HYP
  ,univar_surv_d3$prev_ICNEO
